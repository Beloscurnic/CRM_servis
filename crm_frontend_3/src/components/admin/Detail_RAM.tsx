...
import {AccessoriesClient,Accessories_RAMVm,TypeClient,Type_AccessoriesVm,Get_AccessoriesVm,Update_AccessoriesDto,UpdateClient,CreateClient,Creat_DeliveryDto } from '../../api/api2';
...
const Detail_RAM: FC<{iD_Accessories:string}> = ( children,
   ...
   const [ram, setRam] = useState<Accessories_RAMVm>();

   async function getAccessoroes() {
       const infoRam = await apiClient.Accessories_rAM(parseInt(iD_Accessories!),'2.0');
       setRam(infoRam);
   }
  ...
  const onSubmit = handleSubmit(({quantity_Сomponent, issue_date
      ...
      const delivery: Creat_DeliveryDto = {
          ...
          iD_Сomponent: ram!.iD_Сomponent,
          name_Сomponent:ram!.name_Сomponent,
          type_Сomponent: id_component!.type_Сomponent,
          price_Сomponent: ram!.price,
      ...
      return(
          ...
          <table  className='table'>
              <tr>
                  ...
                  <td> {ram?.iD_Сomponent}</td>
              </tr>
              <tr>
                  <td> Полное название</td>
                  <td> {ram?.name_Сomponent}</td>
              </tr>
              <tr>
                  <td> Тип памяти </td>
              ...
              <tr>
                  <td> Форм-фактор</td>
                  <td> {ram?.form_factor}</td>
              </tr>
              <tr>
                  <td> Ключ модуля памяти</td>
                  <td> {ram?.memory_module_key}</td>
              </tr>
              <tr>
                  <td> Объём </td>
                  <td> {ram?.volume}</td>
