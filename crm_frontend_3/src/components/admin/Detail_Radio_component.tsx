...
import {RadioClient, AccessoriesClient,Radio_componentsVm,TypeClient,Type_AccessoriesVm,Get_AccessoriesVm,Update_AccessoriesDto,UpdateClient,CreateClient,Creat_DeliveryDto } from '../../api/api2';
...
const apiClient5 = new RadioClient('https://localhost:44325');
...
const Detail_Radio_component: FC<{iD_Accessories:string}> = ( children,
   ...
   const [radio_component, setRadio_component] = useState<Radio_componentsVm>();

   async function getAccessoroes() {
       const infoRadio_component = await apiClient5.Accessories_Radio_component(parseInt(iD_Accessories!),'2.0');
       setRadio_component(infoRadio_component);
   }
  ...
  const onSubmit = handleSubmit(({quantity_Сomponent, issue_date
      ...
      const delivery: Creat_DeliveryDto = {
          ...
          iD_Сomponent: radio_component!.iD_Сomponent,
          name_Сomponent:radio_component!.name_Сomponent,
          type_Сomponent: id_component!.type_Сomponent,
          price_Сomponent: radio_component!.price,
      ...
      return(
          ...
          <table  className='table'>
              <tr>
                  ...
                  <td> {radio_component?.iD_Сomponent}</td>
              </tr>
              <tr>
                  <td> Полное название</td>
                  <td> {radio_component?.name_Сomponent}</td>
              </tr>
              <tr>
                  <td> Характеристики </td>
                  <td> {radio_component?.options}</td>
              </tr>
              <tr>
                  <td> Цена</td>
                  <td> {radio_component?.price}</td>
              </tr>
          </table>
          </div>
          <div className='div_header'>
          ...
          <form className="App" action='' onSubmit={onSubmit} >
              <h1 className='h1_creat'>Добавить новые радиодетали </h1>
                  <div>
...
export default Detail_Radio_component