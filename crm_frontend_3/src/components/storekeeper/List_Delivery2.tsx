...
const List_delivery2: FC<{}> = (): ReactElement => {

    const [delivery2, setDelivery] = useState<List_Delivery[]>([]);
        ...
        },
        {
            ...
            Cell: ({ cell }:{cell:any}) => (
                ...
                {cell.row.values.status}{"  "}
                    <div>
                    <button onClick={async()=> {await handleClick(cell.row.values.iD_Delevery!)}}>Закрыть заказ</button>
                    </div>  
                </div>   
            )
        },
